...
export async function getIgnitionReport(selectedId,reportDay) {
  ...
  await axios({
    ...
    searching: false,
    paging: false,
    url: 'http://m1.aragis.com/mobilservice/ajax.php?act=activityIgnition',
    method: 'POST',
    data: formData,
    headers: {
    ...
    .then(
      function (response) {
        ...
        if (data && data.length > 0) {
          ...
          navigationRef.navigate('Content_IgnitionReport');
    ...
    .catch(function (error) {
      console.log('error from getIgnitionReport :', error);
